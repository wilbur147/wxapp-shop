	...
	export default {
		...
		methods: {
			...
			async loadHotKeyword() {
				...
				const result = await request({url: '/program/mall/mallHotKeywords', method: 'GET'});
				if (result && result.code == 200) {
					this.hotKeywordList = result.data;
				}
			}, 
			//监听输入
			async inputChange(event) {
				...
				const res = await request({url: '/program/mall/mallKeyWordsSearch?keyword='+keyword, method: 'GET'});
				if (res && res.code == 200) {
					this.keywordList = [];
